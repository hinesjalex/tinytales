 ...
 * @param {string} [character.description] - Parent-supplied appearance
 *   string from the Character Card, e.g. "with curly long dark brown hair,
 *   medium skin, brown eyes, wearing overalls". When omitted, falls back
 *   to an age-only generic.
 * @returns {string} URL of the character turnaround sheet
 */
export async function generateCharacterReference({ name, age, description }) {
  ...
  const appearance =
    description && description.trim()
      ? description.trim()
      : "with simple, bold, distinctive features that are easy to reproduce consistently";
...
The character is ${ageDesc} named ${name}, ${appearance}.

CRITICAL DESIGN RULES:
- All three views must show the EXACT SAME character with identical features
- Honor the description above: match the specified hair, skin, eyes, and clothing exactly
